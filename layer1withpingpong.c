...
int main() {
    ...
    while (oh < OUTPUT_H) {
        ...
        for (int i = 0; i < HALO_ROWS; i++) {
            int halo_source_slot = BANK_SIZE - HALO_ROWS + i;
            int source_ih = -1;

            if (start_ih_for_inactive_bank != -1) {
                int calculated_ih = start_ih_for_inactive_bank + halo_source_slot;
                if (calculated_ih < INPUT_H) {
                    source_ih = calculated_ih;
                }
            }
