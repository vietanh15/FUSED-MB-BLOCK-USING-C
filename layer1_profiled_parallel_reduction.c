...
// ================== Hàm main ==================
int main() {
    printf("========== LAYER1.C - PARALLEL REDUCTION (NO CRITICAL) ==========\n");
    printf("\n");
    ...
    int theoretical_compute_cycles_sequential = 4718592;
    ...
    printf("  Load = IFM_rows + Weight_rows = %d + %d = %d\n", 
           theoretical_load_ifm_rows, theoretical_weight_per_pe * NUM_PES, theoretical_load_cycles);
    printf("  Compute (parallel) = (32×32×8) × (3×3×4) = 294,912 chu kì\n");
    printf("  [METHOD] Parallel reduction - NO synchronization in PE loop!\n\n");
    printf("======================================================\n\n");
    ...
    // 4. Tính toán và lưu OFM - PARALLEL REDUCTION (NO CRITICAL!)
    printf("[PROFILE] Phase 4: Compute convolution (PARALLEL REDUCTION)...\n");
    ...
    for (int oh = 0; oh < OUTPUT_H; oh++) {
        for (int ow = 0; ow < OUTPUT_W; ow++) {
            for (int f_group = 0; f_group < WEIGHT_BRAM_FILTER_SETS; f_group++) {
                int32_t temp_accum[NUM_PES];  // Temporary local accumulator
                ...
                for (int kh = 0; kh < KERNEL_H; kh++) {
                    for (int kw = 0; kw < KERNEL_W; kw++) {
                        for (int c_chunk = 0; c_chunk < channel_chunks; c_chunk++) {
                            ...
                            // ===== THỰC SỰ SONG PARALLEL: 16 PE chạy đồng thời, MỖI PE CÓ LOCAL ACCUMULATOR =====
                            ...
                            for (int pe = 0; pe < NUM_PES; pe++) {
                                ...
                                // NO SYNCHRONIZATION! Each thread has its own temp_accum[pe]
                                temp_accum[pe] += partial_sum_result;
                            }
                            // Count parallel iterations (not incremented inside parallel region)
                            #pragma omp atomic
                ...
                // Combine results from temp_accum to pe_accumulators
                for (int pe = 0; pe < NUM_PES; pe++) {
                    pe_accumulators[pe] = temp_accum[pe];
                }
                
                // Ghi kết quả vào BRAM OFM
                for (int pe = 0; pe < NUM_PES; pe++) {
    ...
    write_ofm_bram_to_file("ofm_output_parallel_reduction.txt", ofm_bram);
    ...
    printf("\n======================================================\n");
    printf("[PERFORMANCE] Parallel Reduction (NO Critical Section):\n");
    printf("  - Method: Each PE has LOCAL accumulator (temp_accum[pe])\n");
    printf("  - Benefit: NO synchronization bottleneck in PE loop!\n");
    printf("  - Load iterations:     %lld iterations\n", load_iterations);
    printf("  - Compute iterations:  %lld iterations (16 PE × outer loops)\n", compute_loop_iterations);
    printf("  - Compute time:        %.3f ms (16 cores REAL parallel)\n", compute_time);
    printf("  - Total time:          %.3f ms\n\n", total_time);
    
    printf("  Comparison with CRITICAL version:\n");
    printf("  - Expected speedup: ~3-5× faster (no mutex contention)\n");
    printf("  - Compute speedup vs theory: %.2f × (vs 4,718,592 cycles)\n", (4718592 / (compute_time * 1e6)) * 1000);
    printf("======================================================\n");
